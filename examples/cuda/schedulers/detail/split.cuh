  ...
  template <class _SenderId>
    class split_sender_t : sender_base_t {
      ...
      template <class _Receiver>
        using __operation = split::__operation<_SenderId, std::__x<std::remove_cvref_t<_Receiver>>>;

      _Sender __sndr_;
      std::shared_ptr<__sh_state_> __shared_state_;

    public:
      template <std::__decays_to<split_sender_t> _Self, std::execution::receiver _Receiver>
      ...
      template <std::execution::tag_category<std::execution::forwarding_sender_query> _Tag, class... _As _NVCXX_CAPTURE_PACK(_As)>
          requires (!std::__is_instance_of<_Tag, std::execution::get_completion_scheduler_t>) &&
            std::__callable<_Tag, const _Sender&, _As...>
        friend auto tag_invoke(_Tag __tag, const split_sender_t& __self, _As&&... __as)
          ...
          -> std::__call_result_if_t<std::execution::tag_category<_Tag, std::execution::forwarding_sender_query>, _Tag, const _Sender&, _As...> {
      ...
      template <std::__decays_to<split_sender_t> _Self, class _Env>
