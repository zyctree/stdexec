...
TEST_CASE("stopped_as_error simple example", "[adaptors][stopped_as_error]") {
  ...
  auto snd = ex::stopped_as_error(ex::unscoped_transfer(ex::just(11), sched), -1);
...
TEST_CASE("stopped_as_error can we piped", "[adaptors][stopped_as_error]") {
  ...
  ex::sender auto snd = ex::unscoped_transfer(ex::just(11), sched) | ex::stopped_as_error(std::exception_ptr{});
...
TEST_CASE("stopped_as_error using error_code error type", "[adaptors][stopped_as_error]") {
  ...
  ex::sender auto snd = ex::unscoped_transfer(ex::just(11), sched) |
...
TEST_CASE("stopped_as_error keeps values_type from input sender", "[adaptors][stopped_as_error]") {
  ...
  check_val_types<type_array<type_array<int>>>(
      ex::unscoped_transfer(ex::just(23), sched) | ex::stopped_as_error(-1));
  check_val_types<type_array<type_array<double>>>(
      ex::unscoped_transfer(ex::just(3.1415), sched) | ex::stopped_as_error(-1));
}
TEST_CASE("stopped_as_error keeps error_types from input sender", "[adaptors][stopped_as_error]") {
  ...
  check_err_types<type_array<std::exception_ptr>>( //
      ex::unscoped_transfer(ex::just(11), sched1) | ex::stopped_as_error(std::exception_ptr{}));
  check_err_types<type_array<std::exception_ptr>>( //
      ex::unscoped_transfer(ex::just(13), sched2) | ex::stopped_as_error(std::exception_ptr{}));

  check_err_types<type_array<std::exception_ptr, int>>( //
      ex::unscoped_transfer(ex::just(13), sched3) | ex::stopped_as_error(std::exception_ptr{}));
}
...
TEST_CASE("stopped_as_error can add more types to error_types", "[adaptors][stopped_as_error]") {
  ...
  check_err_types<type_array<std::exception_ptr>>( //
      ex::unscoped_transfer(ex::just(11), sched1) | ex::stopped_as_error(-1));
  check_err_types<type_array<std::exception_ptr, int>>( //
      ex::unscoped_transfer(ex::just(13), sched2) | ex::stopped_as_error(-1));

  check_err_types<type_array<std::exception_ptr, int>>( //
      ex::unscoped_transfer(ex::just(13), sched3) | ex::stopped_as_error(-1));

  check_err_types<type_array<std::exception_ptr>>( //
      ex::unscoped_transfer(ex::just(11), sched1)                //
...
TEST_CASE("stopped_as_error overrides sends_stopped to false", "[adaptors][stopped_as_error]") {
  ...
  check_sends_stopped<false>( //
      ex::unscoped_transfer(ex::just(1), sched1) | ex::stopped_as_error(-1));
  check_sends_stopped<false>( //
      ex::unscoped_transfer(ex::just(2), sched2) | ex::stopped_as_error(-1));
  check_sends_stopped<false>( //
      ex::unscoped_transfer(ex::just(3), sched3) | ex::stopped_as_error(-1));
}
