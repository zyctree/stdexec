...
TEST_CASE("when_all completes when children complete", "[adaptors][when_all]") {
  ...
  ex::sender auto snd =                 //
      ex::when_all(                     //
          ex::unscoped_transfer(ex::just(11), sched), //
          ex::unscoped_transfer(ex::just(13), sched), //
          ex::unscoped_transfer(ex::just(17), sched)  //
          )                             //
...
TEST_CASE(
  ...
  ex::sender auto snd = ex::when_all( //
      ex::just(2),                    //
      ex::unscoped_transfer(ex::just(5), sched),    //
      ex::just(7)                     //
  );
  auto op = ex::connect(std::move(snd), expect_error_receiver{});
  ex::start(op);
...
TEST_CASE("when_all cancels remaining children if error is detected", "[adaptors][when_all]") {
  ...
  ex::sender auto snd = ex::when_all(                                //
      ...
      ex::on(sched, ex::unscoped_transfer(ex::just(5), err_sched)),                //
      ex::on(sched, ex::just())                                      //
...
TEST_CASE("when_all cancels remaining children if cancel is detected", "[adaptors][when_all]") {
  ...
  ex::sender auto snd = ex::when_all(                                //
      ex::on(sched, ex::just()) | ex::then([&] { called1 = true; }), //
      ex::on(sched, ex::unscoped_transfer(ex::just(5), stopped_sched)),            //
      ex::on(sched, ex::just())                                      //
          | ex::then([&] { called3 = true; })                        //
          | ex::let_stopped([&] {
              cancelled = true;
