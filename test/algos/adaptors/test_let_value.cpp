...
TEST_CASE("let_value works when changing threads", "[adaptors][let_value]") {
  ...
  bool called{false};
  {
    ...
    ex::sender auto snd = ex::unscoped_transfer(ex::just(7), pool.get_scheduler())                 //
...
TEST_CASE("let_value keeps error_types from input sender", "[adaptors][let_value]") {
  ...
  check_err_types<type_array<std::exception_ptr>>( //
      ex::unscoped_transfer(ex::just(), sched1) | ex::let_value([] { return ex::just(); }));
  check_err_types<type_array<std::exception_ptr>>( //
      ex::unscoped_transfer(ex::just(), sched2) | ex::let_value([] { return ex::just(); }));
  check_err_types<type_array<std::exception_ptr, int>>( //
      ex::unscoped_transfer(ex::just(), sched3) | ex::let_value([] { return ex::just(); }));
}
