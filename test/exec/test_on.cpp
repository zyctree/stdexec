...
TEST_CASE("exec::on transitions back to the receiver's scheduler", "[adaptors][exec::on]") {
  bool called{false};
  auto snd_base = ex::just() | ex::then([&]() -> int {
    called = true;
    return 19;
...
TEST_CASE("inner on transitions back to outer on's scheduler", "[adaptors][exec::on]") {
  bool called{false};
  auto snd_base = ex::just() | ex::then([&]() -> int {
    called = true;
    return 19;
...
TEST_CASE("exec::on(closure) transitions onto and back off of the scheduler", "[adaptors][exec::on]") {
  bool called{false};
...
TEST_CASE("inner on(closure) transitions back to outer on's scheduler", "[adaptors][exec::on]") {
  bool called{false};
